...
self.addEventListener('fetch', event => {
	const dataUrl = 'https://hosp-nckm.github.io/web/notification/';
	event.respondWith(
		caches.match(event.request).then(function (response) {
			return response || fetch(event.request).then(res =>
				// 存 caches 之前，要先打開 caches.open(dataCacheName)
				caches.open('notification_v1')
				.then(function(cache) {
					// cache.put(key, value)
...
self.addEventListener('install', (event) => {
  event.waitUntil(
    caches.open('notification_v1').then((cache) => {
      return cache.addAll([
...
setInterval(Broadcast("check_time"), 60000);
